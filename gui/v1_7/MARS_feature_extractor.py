                ...
                if 'pixel_change_ubbox_mice' in features:
                    ...
                    if f1_bb > 0. or f2_bb > 0.:
                        ...
                        if np.prod(tmp1.shape) > np.prod(tmp2.shape):
                            tmp2 = misc.imresize(tmp2, tmp1.shape)
                        else:
                            tmp1 = misc.imresize(tmp1, tmp2.shape)
                ...
                if 'nose_pc' in features:
                    for s in range(2):
                        ...
                        for p in range(num_points):
                            ...
                            patch2 = misc.imresize(patch2, (max_h, max_w))
                            patch1 = misc.imresize(patch1, (max_h, max_w))
...
def extract_features_top_wnd(features):
    ...
    #     export_feats = izip_longest(*[feats_name], fillvalue='')
