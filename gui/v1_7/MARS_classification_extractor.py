...
def classify_actions_wrapper(top_video_fullpath,
    ...
    try:
        ...
        if (not classifier_savename_exist) | doOverwrite:
            ...
            predicted_labels,predicted_labels_interaction,all_predicted_proba,behaviors = mcm.predict_labels(features, classifier_path)

            # save predictions
            predictions = {'behs_frame':predicted_labels,
                           'interaction_frame':predicted_labels_interaction,
                           'behs_proba':all_predicted_proba[:,:-1],
                           'interaction_proba':all_predicted_proba[:,-1],
                           'behs':behaviors}
            sp.savemat(classifier_savename[:-4]+'.mat',predictions)
            np.savez(classifier_savename[:-4],**predictions)
