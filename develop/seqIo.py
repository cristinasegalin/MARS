...
''' 
Equivalent to Matlab Piotr_toolbox with some extensions to read seq files and manipulate them.
'''
...
#   seqIo_convert( fName, 'convert', tName, imgFun, varargin )
# Replace header of seq file with provided info.
#   seqIo_newHeader( fName, 'newHeader', info )
# Create interface sr for reading dual seq files.
#   sr = seqIo_dualreader( fNames, 'readerDual', [cache] )
# Extract new seq file given a range of frames
#   sr = seqIo_extractor(path_vid,fname_top,fname_front,s,e )
# Convert seq to avi
#   sr = seqIo_toVid(fName,ext='avi' )
...
class seqIo_extractor():
    ...
    def __init__(self,path_vid,vid_top,vid_front,s,e):
        ...
        name =os.path.dirname(vid_top).split('/')[-1]

        if not os.path.exists(path_vid + name + '_%06d_%06d' % (s, e)):
            os.makedirs(path_vid + name + '_%06d_%06d' % (s, e));
        newdir = path_vid + name + '_%06d_%06d' % (s, e);
        ...
        num_frames = sw_top.header['numFrames']
        sw_front = seqIo_writer(video_out_front, sr_front.header)
        num_framesf = sw_front.header['numFrames']
    ...
    def f2(f,num_frames,num_framesf):
            return int(round(f / (num_framesf - 1) * (num_frames - 1))) if num_framesf > num_frames else int(round(f / (num_frames - 1) * (num_framesf - 1)))
