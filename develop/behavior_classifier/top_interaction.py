...
def normalize_pixel_data(data,view):
    fd = [range(40, 49)] if view=='top' else [range(47,67)]
    # fd = [range(39, 49), range(158, 164)] if view=='top' else [47, range(273, 284)] #v15
    fd = list(flatten(fd))
...
def load_data_train(video_list, video_path,ver):
    ...
    for v in video_list:
        vid = dill.load(open(video_path+v+'/output_v1_%d/' % ver +  v +'/' +v + '_raw_feat_top_v1_%d.dill'% ver, 'rb'))
        features = vid['data_smooth']
        ...
        if len(beh['behs_frame'])!=features.shape[0]:
            print(v)

    feats_name = np.array(vid['features'])
    ...
    return data, y, scaler,feats_name
...
for clf_n in range(2):
    for d in range(3):
        ...
        annotator='top_' + dataset + '_' + clf_name + suff  + '/'
        folder = 'mars_v1_6'
        ...
        y_tr = shuffle_back(y_tr, idx_tr).astype(int)

        # 2. test on all of the data
        y_pred_proba = np.zeros((len(y_tr), 2))
        gen = Batch(range(len(y_tr)), lambda x: x % 1e5 == 0, 1e5)
        for i in gen:
            inds = list(i)
        ...
        # do hmm
        hmm_bin = hmm.MultinomialHMM(n_components=2, algorithm="viterbi", random_state=42, params="", init_params="")
        hmm_bin.startprob_ = np.array([np.sum(y_tr == i) / float(len(y_tr)) for i in range(2)])
        hmm_bin.transmat_ = get_transmat(y_tr, 2)
        hmm_bin.emissionprob_ = get_emissionmat(y_tr,y_pred_class, 2)
        ...
        hmm_fbs.emissionprob_ = get_emissionmat(y_tr, y_pred_fbs, 2)
        ...
        _,_,_ = prf_metrics(y_tr, y_pred_class, behs[b])
        _,_,_ = prf_metrics(y_tr, y_pred_hmm, behs[b])
        precision, recall, f_measure = prf_metrics(y_tr, y_pred_fbs_hmm, behs[b])
        ...
        print('########## pd #########')
        prf_metrics(y_te, preds[:,b], behs[b])
        print('########## hmm #########')
        prf_metrics(y_te, preds_hmm[:,b], behs[b])
        print('########## fbs hmm #########')
        prf_metrics(y_te, preds_fbs_hmm[:,b], behs[b])
        ...
        dill.dump(P, open(savedir + 'results_interaction.dill', 'wb'))
